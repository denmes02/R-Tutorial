...
code_checker <- function(label, user_code, check_code, envir_result, evaluate_result, ...) {
  list(message = check_code, correct = TRUE, location = "append")
}
tutorial_options(exercise.timelimit = 10, exercise.checker = code_checker)
...
2.  „Die durchschnittliche Anzahl von Punkten pro Spiel (pts_per_game) unterscheidet sich signifikant zwischen Spielern unterschiedlichen Alters.“
...
# Einfügen erfolgreich war. Hierfür nutzt du head() und dein Variablennamen
# aus Schritt 2.
```
...
# Um zu Filtern musst du der Variable "gefilterte_daten" deine gefilterten Daten 
# übergeben
```
...
## [Datenanalyse]{style="color:#0085FF;"}

In diesem Tutorial lernst du, wie Eigenschaften von Daten aufgerufen werden und wie diese Dargestellt werden. Du wirst Mittelwert, Median, Varianz, Standardabweichung und die Lineare Regression berechnen, sowie Hypothesentests durchführen.

Dafür ist der folgende Ausschnitt des Datensatzes `data` gegeben (Zeile 32-35).




### [**Player Per Game** - Datensatz]{style="color:#005AAE;"}
...
### [Mittelwert]{style="color:#005AAE;"}

* Berechne den Mittelwert mit der Funktion `mean()` von der Spalte `pts_per_game`
...
``` {r mean-hint}
# Über "$" kannst du auf die Spalten aus einem Datensatz zugreifen
data$pf_per_game
```
...
### [Median und Quantile]{style="color:#005AAE;"}

Gegeben ist die Spalte `pts_per_game` aus dem Datensatz `data`.

* Berechne den Median mit der Funktion `median()`.

* Berechne die Quantile Q1, Q2, Q3 mit der Funktion `quantile()`. Die funktion nimmt als erstes Parameter den Datensatz und als zweites die Quantile (25%, 50%, 75%). Übergebe die Quantile in Form eines Vektors, also beispielsweise: `quantileVec <- c(0.25, 0.5, 0.75)`.

* Berechne den Interquartilsabstand mit der Funktion `IQR()`

* Gib den Median und die berechneten Quantile aus.
...
```{r median-solution}
median(data$pts_per_game)
quantile(data$pts_per_game, probs = c(0.25, 0.5, 0.75))
```
...
### [Varianz und Standardabweichung]{style="color:#005AAE;"}

Die Varianz misst die Streuung der Daten um den Mittelwert. Sie ist der Durchschnitt der quadrierten Abweichungen vom Mittelwert:

Eine hohe Varianz bedeutet, dass die Daten weit auseinander liegen, während eine niedrige Varianz darauf hinweist, dass die Daten eng beieinander liegen.

Die Standardabweichung ist die Quadratwurzel der Varianz und gibt die durchschnittliche Abweichung der Datenwerte vom Mittelwert an:

Die Standardabweichung ist ein Maß für die Streuung der Daten. Sie wird oft verwendet, weil sie in denselben Einheiten wie die ursprünglichen Daten gemessen wird, was die Interpretation erleichtert.

* Berechne die Varianz mit `var()` aus dem Datensatz `data` und der Spalte `pts_per_game`.

* Berechne die Standardabweichnung mit `sd()` aus dem Datensatz `data` und der Spalte `pts_per_game`.

* Berechne die Wurzel der Varianz mit `sqrt()`.
...
### [Lineare Regression]{style="color:#005AAE;"}

Die lineare Regression ist eine statistische Methode, mit der die Beziehung zwischen einer abhängigen Variablen (auch Zielvariable genannt) und einer oder mehreren unabhängigen Variablen (auch Prädiktoren oder Merkmale genannt) modelliert wird. Das Ziel der linearen Regression ist es, eine lineare Gleichung zu finden, die die Daten am besten beschreibt.

Die allgemeine Form einer einfachen linearen Regression (mit einer unabhängigen Variablen) lautet:

$y= a+ bx$

Durch diese kann beispielsweise die Funktion

$F(a, b) = \sum_{i=1}^{n} (a + bx_i - y_i)^2 \rightarrow \min$

minimiert werden, indem die Gerade so bestimmt wird, dass Abweichungen zu den Beobachtungspaaren möglichst klein werden.

Die Gerade kann auch in einem Streudiagramm dargestellt werden, welche die Datenpunkte auf einem Streudiagramm so gut wie möglich beschreibt.

```{r,exercise=FALSE, fig.width = 9, fig.height=7}
plot(data$age, data$pts_per_game,
     xlab = "Age", 
     ylab = "Points per game", 
     main = "Streudiagramm - Lineare Regression",
)
abline(lm(data$pts_per_game ~ data$age, data = data))
```


### [Hypothesentests]{style="color:#005AAE;"}

Der Hypothesentest ist ein statistisches Verfahren, um zu entscheiden, ob eine Annahme (Hypothese) über eine Datenmenge auf Basis von Stichprobendaten angenommen oder abgelehnt wird.

Im der Datenbasis wurden Hypothesen aufgestellt. Diese werden hier jetzt durch Hypothesentests bewiesen. 

* Hypothese 1: „**Die Spieler der Boston Celtics werfen diese Saison im Schnitt mehr Dreier pro Spiel als die Spieler der Orlando Magic**“

* Hypothese 2: **Die durchschnittliche Anzahl von Punkten pro Spiel (pts_per_game) unterscheidet sich signifikant zwischen Spielern unterschiedlichen Alters.**

Im folgenden Video findest du eine Anleitung wie du einen Hypothesentest durchführen kannst.

![](https://www.youtube.com/watch?v=3Yz-XNRasGM){width="80%"}

In R kann man den Hypothesen mit dem t-Test beweisen. Dafür können wir die Funktion `t.test()` verwenden. Dabei muss der p-Wert (p-value) kleiner als 0.05 sein, damit die Hypothese erfüllt ist.

So lässt sich [**Hypothese 1**]{style="color:#003260; text-decoration: underline;  text-decoration-thickness: 2px;"} mit `t.test(boston, orlando, var.equal = TRUE, alternative = "greater")` wiederlegen, da für den p-Wert $0.3331>0.05$ raus kommt.

``` {r t-test1, exercise=FALSE, echo=FALSE}
boston <- data$x3p_per_game[data$tm == "BOS"]
orlando <- data$x3p_per_game[data$tm == "ORL"]

# Ensure both vectors have data and are of equal length
if (length(boston) > 0 && length(orlando) > 0) {
  # Limit to the minimum length of the two vectors, if needed
  n <- min(length(boston), length(orlando))
  boston <- boston[1:n]
  orlando <- orlando[1:n]
  
  # Perform the t-test
  t.test(boston, orlando, var.equal = TRUE, alternative = "greater")
} else {
  print("Error: No data available for one or both teams.")
}
```

[**Hypothese 2**]{style="color:#003260; text-decoration: underline;  text-decoration-thickness: 2px;"} lässt sich ebenso mit dem t-Test berechnen:

`t.test(pts_per_game, age, var.equal = TRUE, alternative = "two.sided")`

Es stellt sich heraus, dass die Hypothese wahr ist, da der p-Wert weit unter 0.05 ist.

``` {r t-test2, exercise=FALSE, echo=FALSE}
pts_per_game <- data$pts_per_game
age <- data$age


# Ensure both vectors have data and are of equal length
if (length(pts_per_game) > 0 && length(age) > 0) {
  # Limit to the minimum length of the two vectors, if needed
  n <- min(length(pts_per_game), length(age))
  pts_per_game <- pts_per_game[1:n]
  age <- age[1:n]
  
  describeBy(pts_per_game, age)
...
```{r scatter-plot, exercise=TRUE, echo=FALSE, cache=FALSE, fig.width = 9, fig.height=7}
...
```{r bar-plot, exercise=TRUE, echo=FALSE, cache=FALSE, fig.width = 9, fig.height=7}
...
```{r Mittelwert, echo=FALSE}
  question(
    ...
    answer("Summe aller Werte"),
    answer("Summe aller Werte dividiert durch die Anzahl der Werte", correct = TRUE),
    answer("Der häufigste Wert in der Datenreihe"),
    answer("Der mittlere Wert in einer geordneten Datenreihe")
  )
```
...
```{r Median, echo=FALSE}
  question(
    ...
    answer("Der höchste Wert in der Datenreihe"),
    answer("Der häufigste Wert in der Datenreihe"),
    answer("Der mittlere Wert in einer geordneten Datenreihe", correct = TRUE),
    answer("Die Summe aller Werte dividiert durch die Anzahl der Werte")
  )
```

```{r Varainz, echo=FALSE}
  question(
    "Was misst die Varianz in einer Datenreihe?",
    answer("Die Summe aller Werte"),
    answer("Die Streuung der Datenwerte um den Mittelwert", correct = TRUE),
    answer("Den höchsten Wert in der Datenreihe"),
    answer("Den mittleren Wert in einer geordneten Datenreihe")
  )
```
...
```{r Standartabweichung, echo=FALSE}
  question(
    "Was ist die Standardabweichung einer Datenreihe?",
    answer("Die Quadratwurzel der Varianz", correct = TRUE),
    answer("Die Summe aller Werte dividiert durch die Anzahl der Werte"),
    answer("Der häufigste Wert in der Datenreihe"),
    answer("Der mittlere Wert in einer geordneten Datenreihe")
  )
...
```{r Ergebnispraesentation, echo=FALSE}
...
```{r lineareRegression, echo=FALSE}
  question(
    "Wofür wird die lineare Regression verwendet?",
    answer("Um die Summe aller Werte zu berechnen"),
    answer("Um den mittleren Wert in einer geordneten Datenreihe zu finden"),
    answer("Um die Beziehung zwischen zwei oder mehr Variablen zu modellieren"),
    answer("Um die Streuung der Datenwerte um den Mittelwert zu messen", correct = TRUE)
  )
```
...
#### [Datenanalyse]{style="color:#005AAE;"}

<https://www.youtube.com/watch?v=3Yz-XNRasGM>
